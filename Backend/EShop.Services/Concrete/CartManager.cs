    ...
    public class CartManager : ICartService
    {
        ...
        public async Task<ResponseDto<CartItemDto>> AddToCartAsync(AddToCartDto addToCartDto)
        {
            try
            {
                ...
                var product = await _productRepository.GetAsync(addToCartDto.ProductId);
                if (product == null)
                {
                ...
                var cart = await _cartRepository.GetAsync(
                    predicate: x => x.ApplicationUserId == addToCartDto.ApplicationUserId,
                    includes: query => query.Include(c => c.CartItems!).ThenInclude(ci => ci.Product)
                );
        ...
        public async Task<ResponseDto<CartItemDto>> ChangeQuantityAsync(ChangeQuantityDto changeQuantityDto)
        {
            try
            {
                await _unitOfWork.BeginTransactionAsync();
                var cartItem = await _cartItemRepository.GetAsync(
                    predicate: x => x.Id == changeQuantityDto.CartItemId,
