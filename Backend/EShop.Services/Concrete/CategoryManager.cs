...
namespace EShop.Services.Concrete
{
    public class CategoryManager : ICategoryService
    {
        ...
        public async Task<ResponseDto<CategoryDto>> GetAsync(int id)
        {
            try
            {
                var category = await _categoryRepository.GetAsync(x => x.Id == id && x.IsActive);
                if (category == null)
                {
        ...
        public async Task<ResponseDto<IEnumerable<CategoryDto>>> GetMenuItemAsync()
        {
            try
            {
                var categories = await _categoryRepository.GetAllAsync(predicate: x => x.IsMenuItem & x.IsActive);
                if (!categories.Any())
                {
        ...
        public async Task<ResponseDto<bool>> UpdateIsMenuItemAsync(int id)
        {
            try
            {
                ...
                if (category == null)
                {
                    return ResponseDto<bool>.Fail("Menüde görünme durumu güncellenemedi!", StatusCodes.Status404NotFound);
                }
                ...
                if (result < 1)
                {
                    return ResponseDto<bool>.Fail("Menüde görünme durumu güncellenirken bir hata oluştu!", StatusCodes.Status500InternalServerError);
                }
                return ResponseDto<bool>.Success(category.IsMenuItem, StatusCodes.Status200OK);
            }
            catch (Exception ex)
            {
                return ResponseDto<bool>.Fail(ex.Message, StatusCodes.Status500InternalServerError);
